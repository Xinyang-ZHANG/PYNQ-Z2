...
// ==============================================================
`timescale 1ns/1ps
module system_v_demosaic_0_0_v_demosaic_CTRL_s_axi
#(parameter
    ...
    output wire [15:0]                   HwReg_bayer_phase,
...
localparam
    ...
    ADDR_HWREG_BAYER_PHASE_DATA_0 = 6'h28,
    ADDR_HWREG_BAYER_PHASE_CTRL   = 6'h2c,
    WRIDLE                        = 2'd0,
    WRDATA                        = 2'd1,
    WRRESP                        = 2'd2,
    WRRESET                       = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_HwReg_bayer_phase = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_HWREG_BAYER_PHASE_DATA_0: begin
                    rdata <= int_HwReg_bayer_phase[15:0];
                end
            endcase
        end
    end
...
assign HwReg_bayer_phase = int_HwReg_bayer_phase;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
// int_HwReg_bayer_phase[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_HwReg_bayer_phase[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_HWREG_BAYER_PHASE_DATA_0)
            int_HwReg_bayer_phase[15:0] <= (WDATA[31:0] & wmask) | (int_HwReg_bayer_phase[15:0] & ~wmask);
    end
end
